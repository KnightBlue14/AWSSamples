...
def weather_pull():
    ...
    cur.execute(f"""INSERT INTO leeds (
    Timestamp,
    City,
    Population,
    Country,
    ...
    (stamp,
     str(df4['City'].values[0]),
     int(df4['Population'].values[0]),
     str(df4['Country'].values[0]),
     float(df4['Latitude'].values[0]),
     float(df4['Longitude'].values[0]),
     float(df4['Temperature'].values[0]),
     float(df4['Temp_min'].values[0]),
     float(df4['Temp_max'].values[0]),
     int(df4['Pressure'].values[0]),
     float(df4['Humidity'].values[0]),
     int(df4['Visibility'].values[0]),
     float(df4['Wind_Speed'].values[0]),
     int(df4['Wind_degree'].values[0]),
     float(df4['Rain(1h)'].values[0]),
     float(df4['Rain(3h)'].values[0]),
     float(df4['Snow(1h)'].values[0]),
     float(df4['Snow(3h)'].values[0]),
     str(df4['Weather_ID'].values[0]),
     str(df4['Weather_group'].values[0]),
     str(df4['Description'].values[0]),
     str(df4['Cloudiness'].values[0]),
     rise,
     set,
     str(df4['Air_quality_index'].values[0]),
     str(df4['CO'].values[0]),
     str(df4['NO'].values[0]),
     str(df4['NO2'].values[0]),
     str(df4['O3'].values[0]),
     str(df4['SO2'].values[0]),
     str(df4['NH3'].values[0]),
    str(df4['PM2_5'].values[0]),
...
with DAG('weather_pull_leeds_dag', start_date=datetime(2022, 1, 1), schedule_interval='@daily', catchup=False,tags=['homebrew']) as dag:

    create_db = PostgresOperator(
        task_id = 'create_db',
        sql = """CREATE TABLE IF NOT EXISTS leeds (Timestamp TIMESTAMP PRIMARY KEY, City VARCHAR(255), Population INT, Country VARCHAR(10),Latitude FLOAT, Longitude FLOAT,Temperature FLOAT,Temp_min FLOAT,Temp_max FLOAT,Pressure INT, Humidity INT, Visibility INT, Wind_speed FLOAT,Wind_degree FLOAT, Rain_1h FLOAT,Rain_3h FLOAT, Snow_1h FLOAT,Snow_3h FLOAT, Weather_ID INT, Weather_group VARCHAR(255), Description VARCHAR(255), Cloudiness INT, Sunrise TIMESTAMP, Sunset TIMESTAMP, Air_quality_index INT, CO FLOAT, NO FLOAT, NO2 FLOAT, O3 FLOAT, SO2 FLOAT, NH3 FLOAT, PM2_5 FLOAT, PM10 FLOAT);""",
    ...
    drop_table = PostgresOperator(
        task_id = 'drop_table',
        sql = 'DROP TABLE IF EXISTS leeds;',
        postgres_conn_id = 'aws-postgres'
    ) 


drop_table >> create_db >> pull_weather